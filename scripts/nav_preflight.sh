...
require(lidar.get("frame_id") == "laser_link",
...
print("hardware YAML: mini_mec + rplidar_c1 (460800, laser_link, /scan)")
if mode != "mapping":
...
if mode != "mapping":
    ...
    bt = nav.get("bt_navigator", {}).get("ros__parameters", {})
    require("default_nav_to_pose_bt_xml" not in bt,
            "custom navigate-to-pose BT must be disabled for src_k1 profile")
    require("default_nav_through_poses_bt_xml" not in bt,
            "custom navigate-through-poses BT must be disabled for src_k1 profile")
    require("collision_monitor" not in nav,
            "collision_monitor must be absent from src_k1 profile")
    ...
    controller = nav.get("controller_server", {}).get("ros__parameters", {})
    require(float(controller.get("controller_frequency", 0.0)) == 20.0,
            f"src_k1 controller_frequency={controller.get('controller_frequency')!r}")
    progress_checker = controller.get("progress_checker", {})
    require(progress_checker.get("plugin") == "nav2_controller::SimpleProgressChecker",
            f"progress checker plugin={progress_checker.get('plugin')!r}")
    require(abs(float(progress_checker.get("required_movement_radius", 0.0)) - 0.10) < 1e-6,
            f"movement radius={progress_checker.get('required_movement_radius')!r}")
    require(abs(float(progress_checker.get("movement_time_allowance", 0.0)) - 15.0) < 1e-6,
            f"movement time allowance={progress_checker.get('movement_time_allowance')!r}")
    goal_checker = controller.get("general_goal_checker", {})
    require(abs(float(goal_checker.get("xy_goal_tolerance", 0.0)) - 0.20) < 1e-6,
            f"waypoint position tolerance={goal_checker.get('xy_goal_tolerance')!r}")
    require(abs(float(goal_checker.get("yaw_goal_tolerance", 0.0)) - 0.20) < 1e-6,
            f"specific waypoint yaw tolerance={goal_checker.get('yaw_goal_tolerance')!r}")
    require(abs(float(follow_path.get("max_vel_theta", 0.0)) - 0.50) < 1e-6,
            f"navigation rotation speed={follow_path.get('max_vel_theta')!r}")
    ...
    vy_samples = int(follow_path.get("vy_samples", -1))
    max_vel_y = float(follow_path.get("max_vel_y", 0.0))
    min_vel_y = float(follow_path.get("min_vel_y", 0.0))
    require(vy_samples == 0,
            f"legacy DWB vy_samples={follow_path.get('vy_samples')!r}")
    require(max_vel_y == 0.3 and min_vel_y == -0.3,
            f"legacy DWB lateral limits=({min_vel_y!r}, {max_vel_y!r})")
    require("velocity_smoother" not in nav,
            "legacy profile must not define velocity_smoother parameters")
    local_inflation = nav["local_costmap"]["local_costmap"]["ros__parameters"]["inflation_layer"]
    global_inflation = nav["global_costmap"]["global_costmap"]["ros__parameters"]["inflation_layer"]
    require(float(local_inflation.get("inflation_radius", 0.0)) == 0.15,
            f"legacy local inflation={local_inflation.get('inflation_radius')!r}")
    require(float(global_inflation.get("inflation_radius", 0.0)) == 0.25,
            f"legacy global inflation={global_inflation.get('inflation_radius')!r}")
    print("navigation profile: July legacy DWB; 0.20 m/rad goal, 0.50 rad/s turn, reverse enabled")
else:
