...
turn_on_robot::turn_on_robot():rclcpp::Node ("wheeltec_robot")
{
  ...
  this->get_parameter("odom_topic_name", odom_topic_name);//The odometer topic corresponds to the parent TF coordinate //里程计话题对应父TF坐标
  ...
  odom_publisher = create_publisher<nav_msgs::msg::Odometry>(odom_topic_name, 2);//Create the odometer topic publisher //创建里程计话题发布者
