...
class SileroVAD:
    def __init__(self, model_path="silero_vad.onnx"):
        ...
        self.sess  = ort.InferenceSession(model_path, sess_options=sess_options, providers=['CPUExecutionProvider'])
...
class RecAudioVAD:
    def __init__(self, sld=1, max_time=5, channels=1, rate=48000, device_index=0):
        """
        Args:
            vad_mode: vad 的模式
            sld: 静音多少 s 停止录音
    ...
    def vad_audio(self):
        ...
        try:
            while True:
                ...
                pre_speech_frames.append(frame)  # 每帧都先存入缓冲区
                current_max = np.max(np.frombuffer(frame, dtype=np.short))
                # print('current_max: ', current_max)
